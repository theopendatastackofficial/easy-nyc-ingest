...
class DuckDBWrapper:
    def __init__(self, duckdb_path: str | None = None):
        # If a file path is provided, connect directly to the on-disk DuckDB
        if duckdb_path:
            self.con = duckdb.connect(str(duckdb_path), read_only=False)
        else:
            # In-memory mode: attach the persistent warehouse for analytics
            self.con = duckdb.connect(database=':memory:', read_only=False)
            # Attach on-disk warehouse as 'wh'
            from pipeline.constants import WAREHOUSE_PATH
            self.con.execute(f"ATTACH '{WAREHOUSE_PATH}' AS wh (READ_ONLY)")
            self.con.execute("SET search_path='main,wh'")

        # Enable HTTPFS for external reads
    ...
    def ensure_temp_view(self, asset: str, partitioned: bool = True):
        """
        Guarantee `asset` is available as a TEMP VIEW.
        Preference order: warehouse ‣ local Parquet (recursive, Hive-aware).
        """
        try:
            self.con.execute(
                f"CREATE OR REPLACE TEMP VIEW {asset} AS SELECT * FROM wh.{asset}"
            )
            return
        except duckdb.CatalogException:
            pass  # not in warehouse

        glob_pattern = AssetCatalog.parquet_glob(asset, partitioned=True)
        self.con.execute(
            f"""
            CREATE OR REPLACE TEMP VIEW {asset} AS
    ...
    def query_with_deps(
        ...
        sql: str,
        deps: list[str],
        partitioned: set[str] | None = None
    ) -> pl.DataFrame:
        """
        For each asset in deps, create a temp view,
        then run the given SQL in this connection and return a Polars DF.
        """
        partitioned = partitioned or set()
        for asset in deps:
            self.ensure_temp_view(asset, asset in partitioned)
